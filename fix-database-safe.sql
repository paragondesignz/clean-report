-- Safe database fix script that checks for column existence
...
-- 4. Reports Table (recreate if it exists without user_id)
...
-- Step 2: Add user_id columns to existing tables that might be missing them
...
-- Step 3: Set user_id for existing records (using the first user in auth.users)
DO $$
DECLARE
    first_user_id uuid;
BEGIN
...
-- Step 4: Make user_id columns NOT NULL (only for tables that should have it)
DO $$
BEGIN
    -- Only make NOT NULL if the column exists and has values
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'user_id') THEN
