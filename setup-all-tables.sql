-- Comprehensive database setup script for Clean Report
...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  status VARCHAR(20) DEFAULT 'enquiry' CHECK (status IN ('enquiry', 'scheduled', 'in_progress', 'completed', 'cancelled')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE POLICY "Users can insert their own tasks" ON tasks
  FOR INSERT WITH CHECK (auth.uid() = (SELECT user_id FROM jobs WHERE id = job_id));
CREATE POLICY "Users can update their own tasks" ON tasks
  FOR UPDATE USING (auth.uid() = (SELECT user_id FROM jobs WHERE id = job_id));
CREATE POLICY "Users can delete their own tasks" ON tasks
  FOR DELETE USING (auth.uid() = (SELECT user_id FROM jobs WHERE id = job_id));
...
CREATE POLICY "Users can view their own photos" ON photos
  FOR SELECT USING (auth.uid() = (SELECT user_id FROM jobs WHERE id = (SELECT job_id FROM tasks WHERE id = task_id)));
CREATE POLICY "Users can insert their own photos" ON photos
  FOR INSERT WITH CHECK (auth.uid() = (SELECT user_id FROM jobs WHERE id = (SELECT job_id FROM tasks WHERE id = task_id)));
CREATE POLICY "Users can delete their own photos" ON photos
  FOR DELETE USING (auth.uid() = (SELECT user_id FROM jobs WHERE id = (SELECT job_id FROM tasks WHERE id = task_id)));
